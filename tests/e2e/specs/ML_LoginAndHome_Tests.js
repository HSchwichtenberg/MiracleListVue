...
function CheckLoginForm(browser) {
 browser
  ...
  .assert.textContains("h2", "User Login")
  // Prüfe Bild
...
module.exports = {
 ...
 //  browser.assert.textContains("#errorMsg", "Username and Password required!");
 ...
 "login, create category with tasks and remove": (browser) => {
  ...
  browser.assert.textContains("#categoryCount", 4);
  ...
  browser.assert.textContains("#categoryCount", 5);
  browser.assert.textContains("#categoryCurrentName", newCatname);
  ...
  browser.useXpath().assert.textContains('//*[@id="categoryList"]/li[5]', newCatname);
  ...
  for (var i = 1; i <= 10; i++) {
   ...
   browser.setValue("input[name=newTaskTitle]", newTaskName).sendKeys("input[name=newTaskTitle]", browser.Keys.ENTER).assert.textContains("#taskCount", i); 
   // browser.keys(browser.Keys.ENTER); // funktioniert nicht mehr in Nightwatch nightwatch 2.3.5 :-(
  }
  ...
  browser.useCss().assert.textContains("#categoryCount", 4);
